...
def require_canView
  ...
  def insertWinners
    ...
    @duplicates = Bid.select_by_sql("SELECT * FROM bids WHERE player_id IN (SELECT player_id, user_id, amount, MAX(amount) FROM bids GROUP BY player_id HAVING COUNT(*) > 1)") 
