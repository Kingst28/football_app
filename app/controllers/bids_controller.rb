...
def require_canView
  ...
  def insertWinners
    ...
    @duplicates = Bid.find_by_sql("SELECT * FROM bids WHERE player_id IN (SELECT player_id, user_id, amount, MAX(amount) FROM bids GROUP BY player_id, user_id, amount HAVING COUNT(*) > 1)") 
