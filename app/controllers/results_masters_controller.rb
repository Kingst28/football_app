class ResultsMastersController < ApplicationController
  ...
  def admin_edit
    ...
    @results_masters = ResultsMaster.where(:name => Teamsheet.select(:name).map(&:name)).order(team: :asc, name: :asc)
  ...
  def edit_multiple
    @results_masters = ResultsMaster.where(:id => params[:results_masters_ids]).order(team: :asc, name: :asc)
    @notifications_all = Notification.all
  end
  ...
  def reset_results
    ActiveRecord::Base.connection.execute("update teamsheets set played = false, scored = false, scorenum = 0, conceded = false, concedednum = 0;")
    ActiveRecord::Base.connection.execute("update results_masters set played = false, scored = false, scorenum = 0, conceded = false, concedednum = 0;")
  end
