class ResultsMastersController < ApplicationController
  ...
  def admin_edit
    ...
    @results_masters = ResultsMaster.where(:name => Teamsheet.select(:name).map(&:name))
  ...
  def edit_multiple
    @results_masters = ResultsMaster.joins("INNER JOIN players ON results_masters.name = players.playerteam").includes(:teams).where(:id => params[:results_masters_ids]).order('SUBSTRING_INDEX("players.playerteam", "-", -1)')
    @notifications_all = Notification.all
  end
