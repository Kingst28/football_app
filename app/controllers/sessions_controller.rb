    ...
    for o in @outrightWinners do
      if Teamsheet.exists?(:player_id => o.player_id)
          ...
          if player_position == "Goalkeeper"
            ...
            @bid = Bid.joins("JOIN players ON bids.player_id = players.id").where("bids.user_id = ? AND bids.transfer_out = ? AND bids.account_id = ? AND players.position = ?", o.user_id, true, o.account_id, 'Defender').order(:id)
            if @bid.exists?
              for bid in @bid do 
